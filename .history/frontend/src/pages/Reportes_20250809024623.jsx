...
export default function Reportes() {
  ...
  const auth = lsToken ? `?Authorization=Bearer%20${encodeURIComponent(lsToken)}` : ''
  const xlsxClients = `${backend}/api/clients/export_xlsx${auth}`
  const xlsxInvoices = `${backend}/api/invoices/export_xlsx${auth}`
