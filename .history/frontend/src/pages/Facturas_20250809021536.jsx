...
import { apiGet, apiPost, apiGetBlob, apiDelete, apiPut, apiPatch } from '../lib/api'
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <label className="flex flex-col gap-1">
            ...
            <input className="border border-gray-300 dark:border-gray-600 p-2 rounded focus:outline-none focus:ring-2 focus:ring-brand" name="number" value={form.number} onChange={handleChange} required />
          </label>
          <label className="flex flex-col gap-1">
        ...
        {form.items.map((item, idx) => (
          <div key={idx} className="grid grid-cols-1 sm:grid-cols-4 gap-2 border border-gray-200 dark:border-gray-700 p-2 rounded">
            <label className="flex flex-col gap-1">
              ...
              <input className="border border-gray-300 dark:border-gray-600 p-2 rounded focus:outline-none focus:ring-2 focus:ring-brand" value={item.description} onChange={(e) => handleItemChange(idx, 'description', e.target.value)} required />
            </label>
            <label className="flex flex-col gap-1">
            ...
            <label className="flex flex-col gap-1">
              ...
              <input className="border border-gray-300 dark:border-gray-600 p-2 rounded focus:outline-none focus:ring-2 focus:ring-brand [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" type="number" step="0.01" value={item.unit_price} onChange={(e) => handleItemChange(idx, 'unit_price', e.target.value)} required />
            </label>
            <label className="flex flex-col gap-1">
                ...
                <button key={inv.id} onClick={() => openPreview(inv.id)} className="w-full text-left">
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded">
                    <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 items-center">
                      ...
                      <div className="text-sm text-gray-500 sm:text-center">{inv.date?.slice(0,10)}</div>
                      <div className="font-semibold text-gray-900 dark:text-gray-100 sm:text-right">{(inv.total ?? 0).toFixed(2)} €</div>
