      ...
      <section>
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
            {invoices.map((inv) => (
              <div key={inv.id} className="p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded flex justify-between items-center">
                <span>{inv.number} — {clients.find(c => c.id === inv.client_id)?.name ?? ''}</span>
                <div className="space-x-2">
                  <button onClick={() => downloadInvoice(inv.id, inv.number)} className="text-brand underline">PDF</button>
                </div>
              </div>
            ))}
          </motion.div>
