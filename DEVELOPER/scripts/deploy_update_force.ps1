...
try {
  Write-Log '== Despliegue FORZADO =='
  Set-Location C:\Nioxtec\Nioxtec_Facturer
   = Join-Path C:\Nioxtec\Nioxtec_Facturer 'DEVELOPER\scripts\sync_repo.ps1'
  if (Test-Path ) { &  -HardReset } else { Write-Log 'sync_repo.ps1 no encontrado, fetch+reset --hard'; git fetch origin; =(git rev-parse --abbrev-ref HEAD).Trim(); if (-not ){='main'}; git reset --hard ("origin/"+) }
